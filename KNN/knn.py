...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_test):
    dists[:, i] = torch.sum((train - test[i])**2, dim=1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  train = x_train.reshape(num_train, -1)
  test = x_test.reshape(num_test, -1)
  train_square = torch.sum(train**2, dim=1).view(num_train, -1)
  test_square = torch.sum(test**2, dim=1).view(-1, num_test)
  cross = torch.mm(train, test.t())
  dists = train_square + test_square - 2 * cross
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
    ...
    y_train_tmp = y_train[indices]
    y_pred[i], _ = torch.mode(y_train_tmp, 0)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      ...
      knn_cls = KnnClassifier(x_train, y_train)
      k_to_accuracies[k].append(knn_cls.check_accuracy(x_test, y_test,k))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k, acc_list in k_to_accuracies.items():
    acc_mean = sum(acc_list) / float(len(acc_list))
