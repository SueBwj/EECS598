...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e-3, 1e-2, 1e-1, 0.05, 0.03]
  regularization_strengths = [1e-3, 1e-1, 0.5, 0.4, 0.01]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  ture_predict = y_train_pred == y_train
  train_acc = ture_predict.sum() / y_train.shape[0]

  y_val_pred = cls.predict(X_val)
  ture_val_predict = y_val_pred == y_val
  val_acc = ture_val_predict.sum() / y_val.shape[0]
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    pi = torch.exp(scores[i]) / torch.sum(torch.exp(scores[i]), dim=0)
    loss += -torch.log(pi[y[i]])
    for j in range(num_classes):
      if j == y[i]:
          dW[:, j] += X[i] * (pi[j] - 1)  # 真实类别
      else:
          dW[:, j] += X[i] * pi[j]  # 其他类别
  loss /= num_train
  loss += reg * torch.sum(W * W)
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  softmax = torch.exp(scores) / torch.sum(torch.exp(scores), dim=1, keepdim=True)
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e-3,1e-2,1e-1,1,0.5]
  regularization_strengths = [0.001,0.01,0.1,0,1]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
