...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    loss = torch.sum(-torch.log(output[torch.arange(num_train),y]))
    loss /= num_train
...
def nn_get_search_params():
  ...
  regularization_strengths = [0, 0.0005, 0.001]
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  learning_rate, hidden_size, regularization_strengths, learning_rate_decay = get_param_set_fn()
  for lr in learning_rate:
    for hid_size in hidden_size:
      for reg in regularization_strengths:
        for lr_decay in learning_rate_decay:
          print(f"lr:{lr}, hidden_size:{hid_size},reg:{reg},lr_decay:{lr_decay}")
          net = TwoLayerNet(3 * 32 * 32, hid_size, 10, device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)
          ...
          val_acc = sum(stats['val_acc_history']) / len(stats['val_acc_history'])
