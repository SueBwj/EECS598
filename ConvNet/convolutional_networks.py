        ...
        for i in range(H_out):
            for j in range(W_out):
                x_pad_masked = x_pad[:, :, i*stride:i *
                                     stride+HH, j*stride:j*stride+WW]
                for k in range(F):
                    dw[k, :, :, :] += torch.sum(x_pad_masked *
                                                (dout[:, k, i, j])[:, None, None, None], dim=0)
                for n in range(N):
                    dx_pad[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += torch.sum(
                        (w[:, :, :, :] * (dout[n, :, i, j])[:, None, None, None]), dim=0)
        ...
        for i in range(H_out):
            for j in range(W_out):
                ...
                dx[:, :, i*stride:i*stride+HH, j*stride: j*stride +
                    WW] += mask_filter * (dout[:, :, i, j])[:, :, None, None]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W2'] = weight_scale * torch.randn(
            (H // 2)*(W // 2)*num_filters, hidden_dim, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        conv_forward_out_1, cache_forward_1 = Conv_ReLU_Pool.forward(
            X, self.params['W1'], self.params['b1'], conv_param, pool_param)
        affine_forward_out_2, cache_forward_2 = Linear_ReLU.forward(
            conv_forward_out_1, self.params['W2'], self.params['b2'])
        scores, cache_forward_3 = Linear.forward(
            affine_forward_out_2, self.params['W3'], self.params['b3'])
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        loss += self.reg * (torch.sum(self.params['W1'] ** 2) + torch.sum(
            self.params['W2']**2) + torch.sum(self.params['W3']**2))

        dX3, grads['W3'], grads['b3'] = Linear.backward(dout, cache_forward_3)
        dX2, grads['W2'], grads['b2'] = Linear_ReLU.backward(
            dX3, cache_forward_2)
        dX1, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(
            dX2, cache_forward_1)
...
#             in_channels = C if i == 0 else num_filters[i-1]
#             self.params[f'W{i}'] = weight_scale * torch.randn(int(num_filters[i]), int(
#                 in_channels), filter_size, filter_size, dtype=dtype, device=device)
...
#                 W_out //= 2
#         # 计算展平后的特征维度
#         final_conv_size = num_filters[-1] * H_out * W_out

#         self.params[f"W{len(num_filters)}"] = weight_scale * torch.randn(
#             final_conv_size, num_classes, dtype=dtype, device=device)
#         self.params[f'b{len(num_filters)}'] = torch.zeros(
#             num_classes, dtype=dtype, device=device)
#         ############################################################################
#         #                             END OF YOUR CODE                             #
#         ############################################################################
...
#                 self.bn_params[i][p] = self.bn_params[i][p].type(
#                     dtype).to(device)
...
#             if self.batchnorm and i in self.max_pools:  # Apply BN and pooling
#                 out, cache = Conv_BatchNorm_ReLU_Pool.forward(
#                     out, self.params[f"W{i}"], self.params[f'b{i}'], self.params[f'gamma{i}'],
#                     self.params[f'beta{i}'], conv_param, self.bn_params[i], pool_param)
#             elif not self.batchnorm and i in self.max_pools:  # Apply only pooling
#                 out, cache = Conv_ReLU_Pool.forward(
#                     out, self.params[f'W{i}'], self.params[f'b{i}'], conv_param, pool_param)
#             elif self.batchnorm:  # Apply BN but no pooling
#                 out, cache = Conv_BatchNorm_ReLU.forward(
#                     out, self.params[f'W{i}'], self.params[f'b{i}'], self.params[f'gamma{i}'],
#                     self.params[f'beta{i}'], conv_param, self.bn_params[i])
#             else:  # No BN, no pooling
#                 out, cache = Conv_ReLU.forward(
#                     out, self.params[f'W{i}'], self.params[f'b{i}'], conv_param)
#             caches.append(cache)

#         # 在进行全连接层操作前打印维度信息
#         out_reshaped = out.reshape(out.shape[0], -1)
#         scores, fc_cache = Linear.forward(
#             out_reshaped, self.params[f'W{self.num_layers-1}'], self.params[f'b{self.num_layers-1}'])
...
#             loss += self.reg * \
#                 torch.sum(self.params[f'W{i}'] * self.params[f'W{i}'])

#         dout, grads[f'W{self.num_layers-1}'], grads[f'b{self.num_layers-1}'] = Linear.backward(
#             dout, caches.pop())
#         grads[f'W{self.num_layers-1}'] += 2 * self.reg * \
#             self.params[f'W{self.num_layers-1}']

#         # 逐层反向传播
#         for i in range(self.num_layers - 2, -1, -1):
#             if self.batchnorm and i in self.max_pools:  # BatchNorm + Pooling layer
#                 dout, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = Conv_BatchNorm_ReLU_Pool.backward(
#                     dout, caches.pop())
#             elif not self.batchnorm and i in self.max_pools:  # Pooling layer without BN
#                 dout, grads[f'W{i}'], grads[f'b{i}'] = Conv_ReLU_Pool.backward(
#                     dout, caches.pop())
#             elif self.batchnorm:  # BatchNorm layer without Pooling
#                 dout, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = Conv_BatchNorm_ReLU.backward(
#                     dout, caches.pop())
#             else:  # No BatchNorm, No Pooling
#                 dout, grads[f'W{i}'], grads[f'b{i}'] = Conv_ReLU.backward(
#                     dout, caches.pop())
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i, num_filter in enumerate(num_filters):
            H_out = 1 + \
                (H_out + 2 * conv_param['pad'] - HH) // conv_param['stride']
            W_out = 1 + \
                (W_out + 2 * conv_param['pad'] - WW) // conv_param['stride']

            if self.batchnorm:
                self.params['gamma{}'.format(
                    i)] = 0.01*torch.randn(num_filter, device=device, dtype=dtype)
                self.params['beta{}'.format(
                    i)] = 0.01*torch.randn(num_filter, device=device, dtype=dtype)

            if i in max_pools:
                H_out = 1 + \
                    (H_out - pool_param['pool_height']) // pool_param['stride']
                W_out = 1 + \
                    (W_out - pool_param['pool_width']) // pool_param['stride']

            if weight_scale == 'kaiming':
                self.params['W%d' % i] = kaiming_initializer(
                    num_filter, prev_filters, K=filter_size, relu=True, device=device, dtype=dtype)
            else:
                self.params['W%d' % i] = weight_scale * torch.randn(
                    num_filter, prev_filters, filter_size, filter_size, dtype=dtype, device=device)

            self.params['b%d' % i] = torch.zeros(
        ...
        if weight_scale == 'kaiming':
            self.params['W%d' % i] = kaiming_initializer(
                num_filter*H_out*W_out, num_classes, relu=False, device=device, dtype=dtype)
        else:
            self.params['W%d' % i] = weight_scale * torch.randn(
                num_filter*H_out*W_out, num_classes, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            if i in self.max_pools:
                ...
                if self.batchnorm:
                    ...
                    out, cache['{}'.format(i)] = Conv_ReLU_Pool.forward(out, self.params['W{}'.format(
                        i)], self.params['b{}'.format(i)], conv_param, pool_param)
                ...
                if self.batchnorm:
                    ...
                    out, cache['{}'.format(i)] = Conv_ReLU.forward(
                        out, self.params['W{}'.format(i)], self.params['b{}'.format(i)], conv_param)

        i += 1
        out, cache['{}'.format(i)] = Linear.forward(
            out, self.params['W{}'.format(i)], self.params['b{}'.format(i)])
        scores = out
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        for i in range(self.num_layers):
            if self.batchnorm:
                if i <= (self.num_layers-2):
                    loss += (self.params['gamma{}'.format(i)] *
                             self.params['gamma{}'.format(i)]).sum()*self.reg
        ...
        for i in range(i-1, -1, -1):
            if i in self.max_pools:
                if self.batchnorm:
                    last_dout, dw, db, dgamma1, dbetta1 = Conv_BatchNorm_ReLU_Pool.backward(
                        last_dout, cache['{}'.format(i)])
                    grads['W{}'.format(i)] = dw + 2 * \
                        self.params['W{}'.format(i)]*self.reg
                    grads['b{}'.format(i)] = db
                    grads['beta{}'.format(i)] = dbetta1
                    grads['gamma{}'.format(i)] = dgamma1 + 2 * \
                        self.params['gamma{}'.format(i)]*self.reg
                else:
                    last_dout, dw, db = Conv_ReLU_Pool.backward(
                        last_dout, cache['{}'.format(i)])
                    grads['W{}'.format(i)] = dw + 2 * \
                        self.params['W{}'.format(i)]*self.reg
                    grads['b{}'.format(i)] = db
                ...
                if self.batchnorm:
                    last_dout, dw, db, dgamma1, dbetta1 = Conv_BatchNorm_ReLU.backward(
                        last_dout, cache['{}'.format(i)])
                    grads['W{}'.format(i)] = dw + 2 * \
                        self.params['W{}'.format(i)]*self.reg
                    grads['b{}'.format(i)] = db
                    ...
                    last_dout, dw, db = Conv_ReLU.backward(
                        last_dout, cache['{}'.format(i)])
                    grads['W{}'.format(i)] = dw + 2 * \
                        self.params['W{}'.format(i)]*self.reg
                    grads['b{}'.format(i)] = db
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model,  data_dict, batch_size=128, update_rule=adam,
                    optim_config=optim_config,  device=device, print_every=100)
    ################################################################################
    #                              END OF YOUR CODE                                #
    ################################################################################
    return solver
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        weight_scale = torch.sqrt(torch.tensor(gain / Din, dtype=dtype))
        weight = weight_scale * \
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        weight_scale = torch.sqrt(torch.tensor(
            gain / (Din * K * K), dtype=dtype))
        weight = weight_scale * \
            torch.randn(Din, Dout, K, K, dtype=dtype, device=device)
        ###########################################################################
        #                            END OF YOUR CODE                             #
        ###########################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        N, D = x.shape
        running_mean = bn_param.get('running_mean', torch.zeros(
            D, dtype=x.dtype, device=x.device))
        running_var = bn_param.get('running_var', torch.zeros(
            D, dtype=x.dtype, device=x.device))
        ...
        if mode == 'train':
            ...
            cache = (x, sample_mean, sample_var,
                     x_normalized, gamma, beta, eps)
    ...
    def backward(dout, cache):
        ...
        # Backprop through gamma and beta
        dx_normalized = dout * gamma

        # Compute the gradient of the variance and mean
        dvar = torch.sum(dx_normalized * (x - sample_mean)
                         * -0.5 * torch.pow(sample_var + eps, -1.5), dim=0)
        dmean = torch.sum(dx_normalized * -1 / torch.sqrt(sample_var + eps),
                          dim=0) + dvar * torch.mean(-2.0 * (x - sample_mean), dim=0)
    ...
    def backward_alt(dout, cache):
        ...
        normalization backward pass on paper and simplify as much as possible.
        You should be able to derive a simple expression for the backward pass.
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            sample_var = torch.var(
                x, dim=(0, 2, 3), unbiased=False, keepdim=True)

            x_normalized = (x - sample_mean) / torch.sqrt(sample_var + eps)

            out = gamma.view(1, C, 1, 1) * x_normalized + beta.view(1, C, 1, 1)

            cache = (x, sample_mean, sample_var,
                     x_normalized, gamma, beta, eps)

            running_mean = momentum * running_mean + \
                (1 - momentum) * sample_mean.squeeze()
            running_var = momentum * running_var + \
                (1 - momentum) * sample_var.squeeze()
        elif mode == 'test':
            x_normalized = (x - running_mean.view(1, C, 1, 1)) / \
                torch.sqrt(running_var.view(1, C, 1, 1) + eps)
            out = gamma.view(1, C, 1, 1) * x_normalized + beta.view(1, C, 1, 1)
        else:
    ...
    def backward(dout, cache):
        ...
        # Compute gradients for gamma and beta
        # Summing over N, H, W dimensions
        dgamma = torch.sum(dout * x_normalized, dim=(0, 2, 3))
        # Summing over N, H, W dimensions
        dbeta = torch.sum(dout, dim=(0, 2, 3))

        # Backprop through the normalization
        dx_normalized = dout * gamma.view(1, C, 1, 1)

        # Compute gradient with respect to variance and mean
        dvar = torch.sum(dx_normalized * (x - sample_mean) * -
                         0.5 * torch.pow(sample_var + eps, -1.5), dim=(0, 2, 3))
        dmean = torch.sum(dx_normalized * -1 / torch.sqrt(sample_var + eps), dim=(
            0, 2, 3)) + dvar * torch.mean(-2.0 * (x - sample_mean), dim=(0, 2, 3))

        # Compute gradient with respect to input
        dx = dx_normalized / torch.sqrt(sample_var + eps)

        # Ensure correct broadcasting in dvar term
        dx += (dvar.view(1, C, 1, 1) * 2.0 * (x - sample_mean)) / (N * H * W)
        dx += dmean.view(1, C, 1, 1) / (N * H * W)
        ###########################################################################
        #                             END OF YOUR CODE                            #
        ###########################################################################
...
class FastConv(object):
    ...
    def backward(dout, cache):
        try:
            ...
            dx, dw, db = torch.zeros_like(tx), torch.zeros_like(
                layer.weight), torch.zeros_like(layer.bias)
        return dx, dw, db
...
class FastMaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        layer = torch.nn.MaxPool2d(kernel_size=(
            pool_height, pool_width), stride=stride)
        tx = x.detach()
        tx.requires_grad = True
        out = layer(tx)
