...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.params['W1'] = weight_scale * \
            torch.randn(input_dim, hidden_dim, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        out1, fc1_relu_cache = Linear_ReLU.forward(
            X, self.params['W1'], self.params['b1'])
        out2, fc2_cache = Linear.forward(
        ...
        loss += self.reg * (torch.sum(self.params['W1'] * self.params['W1']) + torch.sum(
            self.params['W2'] * self.params['W2']))

        dout, grads['W2'], grads['b2'] = Linear.backward(dout, fc2_cache)
        _, grads['W1'], grads['b1'] = Linear_ReLU.backward(
            dout, fc1_relu_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        # Replace "pass" statement with your code
        self.params['W0'] = weight_scale * \
            torch.randn(input_dim, hidden_dims[0], dtype=dtype, device=device)
        self.params['b0'] = torch.zeros(
        ...
        self.params[f'W{len(hidden_dims)}'] = weight_scale * torch.randn(
            hidden_dims[-1], num_classes, dtype=dtype, device=device)
        self.params[f'b{len(hidden_dims)}'] = torch.zeros(
            num_classes, dtype=dtype, device=device)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            out, fc_relu_cache = Linear_ReLU.forward(
                out, self.params[f'W{i}'], self.params[f'b{i}'])
            cache.append(fc_relu_cache)
            if self.use_dropout:
                out, droupout_cache = Dropout.forward(
                    out, self.dropout_param)
                cache.append(droupout_cache)

        out2, fc_cache = Linear.forward(
            out, self.params[f'W{self.num_layers-1}'], self.params[f'b{self.num_layers-1}'])
        ...
        for i in range(self.num_layers-2, -1, -1):
            if self.use_dropout:
                dout = Dropout.backward(dout, cache.pop())
            dout, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(
                dout, cache.pop())
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    optim_config = {'lr': 0.01, 'reg': 0.01}
    solver = Solver(model, data_dict,
                    optim_config=optim_config, num_epochs=50, device=device,)
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return solver
...
def adam(w, dw, config=None):
    ...
    next_w = w - config['learning_rate'] * m_hat / \
        (torch.sqrt(v_hat) + config['epsilon'])
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
