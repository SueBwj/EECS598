...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        self.mu_layer = nn.Linear(
            in_features=self.hidden_dim, out_features=self.latent_size)
    ...
    def forward(self, x):
        ...
        mu = self.mu_layer(encoder_x)
        logvar = self.logvar_layer(encoder_x)
...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        self.hidden_dim = 256
        self.encoder = nn.Sequential(
            nn.Linear(in_features=self.input_size + self.num_classes,
                      out_features=self.hidden_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim,
                      out_features=self.hidden_dim),
        ...
        # Replace "pass" statement with your code
        self.decoder = nn.Sequential(
            nn.Linear(in_features=self.latent_size + self.num_classes,
                      out_features=self.hidden_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim,
                      out_features=self.hidden_dim),
    ...
    def forward(self, x, c):
        ...
        x_input = torch.cat((x_flatten, c), dim=1)
        encoder_x = self.encoder(x_input)
        mu = self.mu_layer(encoder_x)
        logvar = self.logvar_layer(encoder_x)
